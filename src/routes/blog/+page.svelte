...
<section class="mx-auto max-w-content px-6 py-20 lg:px-10">
	<div class="grid gap-6 sm:grid-cols-2">
		{#each T.blog.posts as post (post.title)}
			<article class="reticle group border border-line bg-paper p-8 transition-colors hover:border-gold">
				<div class="flex items-center justify-between text-xs">
					...
					<span class="text-ink/40">{post.date}</span>
				</div>
				<h2 class="mt-4 text-xl font-semibold leading-snug text-ink">{post.title}</h2>
				<p class="mt-3 text-sm leading-relaxed text-ink/55">{post.excerpt}</p>
